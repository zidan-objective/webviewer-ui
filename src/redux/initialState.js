...
export default options => ({
  viewer: {
    ...
    activeLeftPanel: options.hideAnnotationPanel || !options.a || options.readonly ? 'thumbnailsPanel' : 'notesPanel',
    activeToolGroup: '',
    expandedNotes: {},
    notePopupId: '',
    isNoteEditing: false,
    ...
    isFullScreen: false,
    doesAutoLoad: options.auto_load === false ? false : true,
  ...
  document: {
    id: options.did || null,
    initialDoc: options.initialDoc || options.d || '',
    path: null,
    filename: null,
    file: null,
    type: null,
    pdfDoc: null,
    pdfType: options.pdf || 'wait',
    officeType: options.office || 'wait',
    isOffline: options.startOffline || false,
    totalPages: 0,
    outlines: [],
    checkPassword: null,
    password: '',
  ...
  advanced: {
    azureWorkaround: options.azureWorkaround || false,
    customCSS: options.css || null,
    customData: options.custom || null,
    customHeaders: { },
    configScript: options.config || '',
    defaultDisabledElements: options.disabledElements || '',
    externalPath: options.p || '',
    engineType: documentTypeParamToEngineType(options.documentType),
    fullAPI: options.pdfnet || false,
    pdftronServer: options.pdftronServer || '',
    disableWebsockets: options.disableWebsockets || false,
    preloadWorker: options.preloadWorker || false,
    serverUrl: options.server_url || '',
    serverUrlHeaders: JSON.parse(options.serverUrlHeaders || '{}'),
    streaming: options.streaming || false,
    subzero: options.subzero || false,
    useDownloader: options.useDownloader || true,
    useSharedWorker: options.useSharedWorker || false,
    disableI18n: options.disableI18n || false,
